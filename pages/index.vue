  ...
  <main class="site-shell">
    ...
    <section id="mapa" class="asterion-map-section section-pad" data-progress-tone="dark">
      ...
      <div class="map-explorer" data-reveal data-reveal-delay="110">
        <div class="map-canvas"><img data-parallax="0.035" :src="art.map" alt="Mapa de expedição de Asterion com rotas entre Ferrosul, Nareth, Asterion e Casa dos Ecos" /><svg class="map-route-lines" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><defs><mask id="home-route-reveal"><path class="route-mask" d="M22 31 C38 16 63 16 76 25" /><path class="route-mask" d="M76 25 S70 55 58 69" /><path class="route-mask" d="M58 69 S37 78 26 73" /></mask></defs><g mask="url(#home-route-reveal)"><path class="route-ghost" d="M22 31 C38 16 63 16 76 25 S70 55 58 69 S37 78 26 73" /><path class="route-pulse route-pulse-one" d="M22 31 C38 16 63 16 76 25" /><path class="route-pulse route-pulse-two" d="M76 25 S70 55 58 69" /><path class="route-pulse route-pulse-three" d="M58 69 S37 78 26 73" /></g></svg><span class="map-caption">ARQ. VIG · percurso estimado</span><button v-for="(region, index) in mapRegions" :key="region.name" type="button" class="map-hotspot" :class="[{ active: activeMapRegion === index }, `hotspot-${region.tone}`]" :style="{ left: region.x, top: region.y }" :aria-label="`Selecionar ${region.name}`" :aria-describedby="`map-tooltip-${index}`" @click="selectMapRegion(index)"><i /><span class="map-hotspot-label">{{ region.name }}</span><span :id="`map-tooltip-${index}`" class="map-tooltip" role="tooltip"><small>{{ region.note }}</small><b>{{ region.tooltip }}</b><em>Selecionar registro <span>↗</span></em></span></button></div>
        <article :key="selectedMapRegion.name" class="map-record" :class="`map-${selectedMapRegion.tone}`" aria-live="polite"><img :src="selectedMapRegion.image" :alt="selectedMapRegion.name" /><div><p>{{ selectedMapRegion.note }}</p><h3>{{ selectedMapRegion.name }}</h3><p class="map-description">{{ selectedMapRegion.description }}</p><button type="button" @click="openMapModal">Abrir registro completo <span>↗</span></button></div></article>
      </div>
    </section>
