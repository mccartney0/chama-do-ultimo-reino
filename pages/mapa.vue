...
<template>
  <main class="map-page-shell">
    ...
    <section class="map-page-intro" data-reveal><div><p class="eyebrow"><span />Carta de expedição · Livro I</p><h1>A rota não aponta<br />o destino. <em>Ela o convoca.</em></h1></div><p>Um registro ampliado da travessia entre as forjas de Ferrosul, as ruínas de Nareth, a cidade quebrada de Asterion e a Casa dos Ecos.</p></section>

    <section class="route-atlas" aria-label="Carta interativa das rotas de Asterion" data-reveal data-reveal-delay="90">
      <div class="route-atlas-canvas"><img data-parallax="0.035" :src="art.map" alt="Carta ampliada de Asterion com a rota de Ferrosul à Casa dos Ecos" /><div class="atlas-grain" data-parallax="0.018" aria-hidden="true" /><svg class="atlas-route-lines" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><defs><mask id="atlas-route-reveal"><path class="route-mask" d="M22 31 C38 16 63 16 76 25" /><path class="route-mask" d="M76 25 S70 55 58 69" /><path class="route-mask" d="M58 69 S37 78 26 73" /></mask></defs><g mask="url(#atlas-route-reveal)"><path class="atlas-route-underlay" d="M22 31 C38 16 63 16 76 25 S70 55 58 69 S37 78 26 73" /><path class="atlas-route atlas-route-one" d="M22 31 C38 16 63 16 76 25" /><path class="atlas-route atlas-route-two" d="M76 25 S70 55 58 69" /><path class="atlas-route atlas-route-three" d="M58 69 S37 78 26 73" /></g></svg><span class="atlas-index">ARQ. VIG · 04 pontos</span><button v-for="(region, index) in regions" :key="region.name" type="button" class="atlas-hotspot" :class="[{ active: activeRegion === index }, `hotspot-${region.tone}`]" :style="{ left: region.x, top: region.y }" :aria-label="`Ler o registro de ${region.name}`" :aria-describedby="`atlas-tooltip-${index}`" @click="activeRegion = index"><i /><span class="atlas-hotspot-label"><b>{{ region.number }}</b>{{ region.name }}</span><span :id="`atlas-tooltip-${index}`" class="atlas-tooltip" role="tooltip"><small>{{ region.note }}</small><strong>{{ region.tooltip }}</strong><em>Selecionar registro <span>↗</span></em></span></button></div>
